...
library LiquidVoting {
    ...
    function updateVote(LiquidVoting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 balance, uint256 totalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
        return _update(self, user, oldVote, newVote, balance, balance, totalSupply, defaultVote, emitEvent);
    }

    function updateBalance(LiquidVoting.Data storage self, address user, Vote.Data memory oldVote, uint256 oldBalance, uint256 newBalance, uint256 newTotalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
    ...
    function _update(LiquidVoting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 oldBalance, uint256 newBalance, uint256 newTotalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) private {
        ...
        }
        {
            ...
            if (newResult != data.result) {
                ...
                (sdata.oldResult, sdata.result, sdata.time) = (data.current().toUint104(), newResult.toUint104(), block.timestamp.toUint48());
            }
        }
...
library MooniswapConstants {
    ...
    uint256 internal constant _MIN_REFERRAL_SHARE = 0.05e18;
    ...
    uint256 internal constant _MAX_SHARE = 0.1e18;
    uint256 internal constant _MAX_DECAY_PERIOD = 5 minutes;
    uint256 internal constant _DEFAULT_FEE = 0;
    uint256 internal constant _DEFAULT_SLIPPAGE_FEE = 1e18;
    uint256 internal constant _DEFAULT_REFERRAL_SHARE = 0.1e18;
...
library VirtualBalance {
    ...
    function set(VirtualBalance.Data storage self, uint256 balance) internal {
        (self.balance, self.time) = (balance.toUint216(), block.timestamp.toUint40());
    }
    ...
    function current(VirtualBalance.Data memory self, uint256 decayPeriod, uint256 realBalance) internal view returns (uint256) {
        ...
        return uint256(self.balance).mul(timeRemain).add(realBalance.mul(timePassed)).div(decayPeriod);
    }
}
...
library VirtualVote {
    ...
    function current(VirtualVote.Data memory self) internal view returns (uint256) {
        ...
        return uint256(self.oldResult).mul(timeRemain).add(uint256(self.result).mul(timePassed)).div(_VOTE_DECAY_PERIOD);
    }
}
...
library Voting {
    ...
    function updateVote(Voting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 balance, uint256 totalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
        return _update(self, user, oldVote, newVote, balance, balance, totalSupply, defaultVote, emitEvent);
    }

    function updateBalance(Voting.Data storage self, address user, Vote.Data memory oldVote, uint256 oldBalance, uint256 newBalance, uint256 newTotalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
        return _update(self, user, oldVote, (newBalance == 0) ? Vote.init() : oldVote, oldBalance, newBalance, newTotalSupply, defaultVote, emitEvent);
    }

    function _update(Voting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 oldBalance, uint256 newBalance, uint256 newTotalSupply, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) private {
        uint256 oldWeightedSum = self._weightedSum;
        uint256 newWeightedSum = oldWeightedSum;
        uint256 oldDefaultVotes = self._defaultVotes;
        uint256 newDefaultVotes = oldDefaultVotes;
...
contract BalanceAccounting {
    ...
    function _set(address account, uint256 amount) virtual internal returns (uint256 oldAmount) {
...
library ExplicitLiquidVoting {
    ...
    function updateVote(ExplicitLiquidVoting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 balance, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
        return _update(self, user, oldVote, newVote, balance, balance, defaultVote, emitEvent);
    }

    function updateBalance(ExplicitLiquidVoting.Data storage self, address user, Vote.Data memory oldVote, uint256 oldBalance, uint256 newBalance, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) internal {
        return _update(self, user, oldVote, (newBalance == 0) ? Vote.init() : oldVote, oldBalance, newBalance, defaultVote, emitEvent);
    }

    function _update(ExplicitLiquidVoting.Data storage self, address user, Vote.Data memory oldVote, Vote.Data memory newVote, uint256 oldBalance, uint256 newBalance, uint256 defaultVote, function(address, uint256, bool, uint256) internal emitEvent) private {
        uint256 oldWeightedSum = self._weightedSum;
        uint256 newWeightedSum = oldWeightedSum;
...
abstract contract MooniswapGovernance is ERC20, Ownable, ReentrancyGuard {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) override internal {
        ...
        ParamsHelper memory params = ParamsHelper({from: from, to: to, updateFrom: updateFrom, updateTo: updateTo, amount: amount, balanceFrom: balanceFrom, balanceTo: balanceTo, newTotalSupply: newTotalSupply});
    ...
    function _updateOnTransfer(ParamsHelper memory params, uint256 defaultValue, function(address, uint256, bool, uint256) internal emitEvent, LiquidVoting.Data storage votingData) private {
...
abstract contract BaseGovernanceModule is IGovernanceModule {
    ...
    function notifyStakesChanged(address[] calldata accounts, uint256[] calldata newBalances) override external onlyMothership() {
    ...
    function notifyStakeChanged(address account, uint256 newBalance) override external onlyMothership() {
        _notifyStakeChanged(account, newBalance);
    }

    function _notifyStakeChanged(address account, uint256 newBalance) virtual internal;
}
...
contract MooniswapFactoryGovernance is IMooniswapFactoryGovernance, BaseGovernanceModule, BalanceAccounting, Ownable, Pausable {
    ...
    function shareParameters() override external view returns (uint256, uint256, address, address) {
        return (_referralShare.data.current(), _governanceShare.data.current(), governanceWallet, feeCollector);
    }

    function defaults() override external view returns (uint256, uint256, uint256) {
        return (_defaultFee.data.current(), _defaultSlippageFee.data.current(), _defaultDecayPeriod.data.current());
    }

    function defaultFee() override external view returns (uint256) {
    ...
    function virtualDefaultFee() override external view returns (uint104, uint104, uint48) {
        return (_defaultFee.data.oldResult, _defaultFee.data.result, _defaultFee.data.time);
    }

    function defaultSlippageFee() override external view returns (uint256) {
    ...
    function virtualDefaultSlippageFee() override external view returns (uint104, uint104, uint48) {
        return (_defaultSlippageFee.data.oldResult, _defaultSlippageFee.data.result, _defaultSlippageFee.data.time);
    }

    function defaultDecayPeriod() override external view returns (uint256) {
    ...
    function virtualDefaultDecayPeriod() override external view returns (uint104, uint104, uint48) {
        return (_defaultDecayPeriod.data.oldResult, _defaultDecayPeriod.data.result, _defaultDecayPeriod.data.time);
    }

    function referralShare() override external view returns (uint256) {
    ...
    function governanceShare() override external view returns (uint256) {
    ...
    function _notifyStakeChanged(address account, uint256 newBalance) override internal {
...
contract Mooniswap is MooniswapGovernance {
    ...
    event Deposited(address indexed sender, address indexed receiver, uint256 share, uint256 token0Amount, uint256 token1Amount);

    event Withdrawn(address indexed sender, address indexed receiver, uint256 share, uint256 token0Amount, uint256 token1Amount);
    ...
    constructor(IERC20 _token0, IERC20 _token1, string memory name, string memory symbol, IMooniswapFactoryGovernance _mooniswapFactoryGovernance) public ERC20(name,symbol) MooniswapGovernance(_mooniswapFactoryGovernance) {
    ...
    function swapFor(IERC20 src, IERC20 dst, uint256 amount, uint256 minReturn, address referral, address payable receiver) public payable nonReentrant() whenNotShutdown() returns (uint256 result) {
        ...
        Balances memory balances = Balances({src: src.uniBalanceOf(address(this)).sub(src.isETH() ? msg.value : 0), dst: dst.uniBalanceOf(address(this))});
        uint256 confirmed;
        Balances memory virtualBalances;
        Fees memory fees = Fees({fee: fee(), slippageFee: slippageFee()});
    ...
    function _doTransfers(IERC20 src, IERC20 dst, uint256 amount, uint256 minReturn, address payable receiver, Balances memory balances, Fees memory fees) private returns (uint256 confirmed, uint256 result, Balances memory virtualBalances) {
        uint256 _decayPeriod = decayPeriod();
...
contract BaseRewards is Ownable, BalanceAccounting {
    ...
    function rewardPerToken(uint i) public view returns (uint256) {
        ...
        return tr.rewardPerTokenStored.add(lastTimeRewardApplicable(i).sub(tr.lastUpdateTime).mul(tr.rewardRate).div(totalSupply()));
    }
    ...
    function _earned(uint i, address account, uint256 _rewardPerToken) private view returns (uint256) {
        TokenRewards storage tr = tokenRewards[i];
        return balanceOf(account).mul(_rewardPerToken.sub(tr.userRewardPerTokenPaid[account])).div(tr.scale).add(tr.rewards[account]);
    }
}
