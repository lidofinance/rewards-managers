...
def test_deposit(
    ...
    assert staking_rewards_sushi.totalSupply() == DEPOSIT_AMOUNT
    assert staking_rewards_sushi.balanceOf(ape) == DEPOSIT_AMOUNT
    assert staking_rewards_sushi.rewards(ape) == 0
    assert staking_rewards_sushi.userRewardPerTokenPaid(ape) == 0
...
def test_withdraw(
    ...
    assert staking_rewards_sushi.balanceOf(ape) == DEPOSIT_AMOUNT // 2
    assert staking_rewards_sushi.balanceOf(stranger) == 0

    # validate that reward and earned was reset
    assert staking_rewards_sushi.rewards(ape) == 0
    assert staking_rewards_sushi.earned(ape) == 0


@pytest.mark.usefixtures("notify_reward_amount_sushi")
def test_harvest_no_rewards(
    ape, stranger, master_chef_v2, staking_rewards_sushi, ldo_token
):
    # test some preconditions
    assert ldo_token.balanceOf(stranger) == 0
...
def test_withdraw_and_harvest_empty_balance(
    ...
    # validate that balance was updated correctly
    assert staking_rewards_sushi.balanceOf(ape) == 0
    assert staking_rewards_sushi.balanceOf(stranger) == 0
